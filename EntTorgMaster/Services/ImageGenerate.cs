...
namespace EntTorgMaster.Services
{
    public static class ImageGenerate
    {
        ...
        private static Pen linePen = new Pen(Color.Black, 2);
        public static void Create(OrderDoor orderDoor)
        {
            Door door = new Door(orderDoor);
            door.FramugaH = door.FramugaH == null ? null : 200 * door.FramugaH / door.H;
            door.W = 200 * door.W / door.H;
            door.H = 200;
            door.S = door.S == null ? null : 200 * door.S / door.H;
            if(door.SEqual)
                door.S = (200 * door.W /2) / door.H;
            //door.H = door.H - door.FramugaH ?? 0;
            using (Image<La32> img = new Image<La32>(door.W+20, (door.H + 20) + door.FramugaH ?? 0))
            {
